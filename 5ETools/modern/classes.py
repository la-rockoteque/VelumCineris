...
# Note: Using pd.read_csv with header=1 for these sheets requires direct URL
# If these sheets need header=1, you may need to fetch them differently
import io
import requests

def get_sheet_with_header(gid, header=1):
    url = modern_sheets._build_csv_url(gid)
    response = requests.get(url)
    return pd.read_csv(io.StringIO(response.text), header=header)

df_classes = get_sheet_with_header("1924660120", header=1)
df_classes.head()

df_class_tables = get_sheet_with_header("193036738", header=1)
df_class_tables.head()

df_class_features = get_sheet_with_header("545140625", header=1)
