...
def get_species_sheet():
    return species_dataset.get_species_sheet("modern")


def row_to_species(row, *, df_species):
    return species_dataset.row_to_species(row, setting="modern", df_species=df_species)
...
def build_races_list(source_code: str | None = None) -> list[dict]:
    return species_dataset.build_races_list(setting="modern", source_code=source_code)
