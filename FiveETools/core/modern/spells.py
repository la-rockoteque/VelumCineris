...
from FiveETools.mappers.spell_mapper import map_modern_spell_row
...
def _parse_spell_level(value: Any) -> int:
    if pd.isnull(value):
        return 0
...
def row_to_spell(row, *, json_source: str) -> dict[str, Any]:
    return map_modern_spell_row(
        row,
        json_source=json_source,
        default_source=source_catalog.DEFAULT_SOURCE,
        parse_spell_level_fn=_parse_spell_level,
    )


def get_converter() -> SpellConverter:
    return cast(SpellConverter, get_dataset_converter("spells", setting="modern"))
...
def build_spells_pydantic(source_code: str | None = None) -> list[Spell]:
    return cast(
        list[Spell], load_dataset("spells", source_code=source_code, setting="modern")
    )
