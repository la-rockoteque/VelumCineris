...
export function NamedTableField(props: NamedTableFieldProps) {
  ...
  const entries = useMemo(() => parseNamedEntries(rawValue), [rawValue]);
  ...
  const updateEntry = (index: number, patch: Partial<{ key: string; value: string }>) => {
    ...
    commit(serializeNamedEntries(copy));
  };

  const addRow = () => {
    commit(serializeNamedEntries([...entries, { key: "", value: "" }]));
  };

  const removeRow = (index: number) => {
    const copy = entries.filter((_, idx) => idx !== index);
    commit(serializeNamedEntries(copy.length ? copy : [{ key: "", value: "" }]));
  };
