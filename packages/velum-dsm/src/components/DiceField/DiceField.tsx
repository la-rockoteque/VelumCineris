...
export function DiceField(props: DiceFieldProps) {
    ...
    const parsed = useMemo(() => parseDiceValue(rawValue), [rawValue]);
    ...
    const onCountChange = (event: ChangeEvent<HTMLInputElement>) => {
        const next = stringifyDiceValue({ ...parsed, count: safeNumber(event.target.value, 0) });
        setRawValue(next);
        props.onChange(next);
    };

    const onDiceTypeChange = (event: ChangeEvent<HTMLSelectElement>) => {
        const next = stringifyDiceValue({ ...parsed, diceType: safeNumber(event.target.value, 0) });
        setRawValue(next);
        props.onChange(next);
    };

    const onBonusChange = (event: ChangeEvent<HTMLInputElement>) => {
        const next = stringifyDiceValue({ ...parsed, bonus: safeNumber(event.target.value, 0) });
        setRawValue(next);
        props.onChange(next);
    };
